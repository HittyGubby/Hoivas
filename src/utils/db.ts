...
export class AppDatabase {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event: any) => {
        ...
        if (!db.objectStoreNames.contains("customPics")) {
          const s = db.createObjectStore("customPics", {
            keyPath: "id",
            autoIncrement: true,
          });
  ...
  private async serializeBlobUrls(nodes: any[]): Promise<any[]> {
    ...
    for (const node of snapshot) {
      ...
      for (const k of keys) {
        if (
          typeof node.data[k] === "string" &&
          node.data[k].startsWith("blob:")
        ) {
  ...
  async deserializeBlobUrls(nodes: any[]): Promise<any[]> {
    ...
    for (const node of snapshot) {
      ...
      for (const k of keys) {
        if (
          typeof node.data[k] === "string" &&
          node.data[k].startsWith("__custom_pic__:")
        ) {
      ...
      if (node.data.spirits) {
        for (const s of node.data.spirits) {
          if (
            typeof s.url === "string" &&
            s.url.startsWith("__custom_pic__:")
          ) {
  ...
  private async getUrlForPicId(id: number): Promise<string | null> {
    ...
    return new Promise((resolve) => {
      const request = db
        .transaction(["customPics"], "readonly")
        .objectStore("customPics")
        .get(id);
      request.onsuccess = () => {
  ...
  async saveAutosave(data: any) {
    ...
    const serializedFocusNodes = await this.serializeBlobUrls(
      data.focusNodes || [],
    );
    return new Promise((resolve, reject) => {
  ...
  async addCustomPic(type: string, filename: string, blob: Blob) {
    ...
    const maxOrder =
      pics.length > 0 ? Math.max(...pics.map((p) => p.order || 0)) : 0;
  ...
  async deleteCustomPic(id: number) {
    ...
    db.transaction(["customPics"], "readwrite")
      .objectStore("customPics")
      .delete(id);
  }
